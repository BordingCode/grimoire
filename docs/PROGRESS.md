...
- **v55:** **Proficiencies & Languages → add-one-by-one chips** (like weapons). Each topic (Languages/Armor/Weapons/Tools) = a chip list; "+ add" opens `profForm(field)` with a native `<datalist>` of SRD-name suggestions (already-added filtered out) + free custom typing; Enter/Add dedupes case-insensitively & keeps the form open; ✕ removes. `PROF_TOPICS`/`profList` (read, legacy-string tolerant) in views.js; `profArray` (write migrate) + `profAddOpen/profAdd/profRemove` in app.js. `ch.proficiencies` is now `{languages:[],armor:[],weapons:[],tools:[]}` (legacy comma-strings migrate lazily). Synced via mental link group. Also: **removed the leftover "roll" label** on weapon rows (dice removed back in v45; tap = info modal) → now a "›" chevron.
- **v56:** spell look-up link now points to the **D&D 5e Wikidot** (`https://dnd5e.wikidot.com/spell:<slug>`, `wikidotSpellUrl()` in util.js) instead of a D&D Beyond search — on the greyed stubs, full spell detail, and hand-add form. **`parseSpellsText` now handles the Wikidot paste format**: captures the "Spell Lists. Bard, Cleric, …" line → spell's class list (not just the character's class), keeps it out of the description / "At Higher Levels", ignores an in-block "Source:" line, detects ritual from casting time. Verified parsing real Wikidot Fireball / Fire Bolt / Hold Person.
- **v57 fixes:** (1) **Coins reset to 0 on update** — `currency` was in the shared `gear` link group, so a linked character overwrote local coins with the partner's (default-0) snapshot on every boot/pull. Removed `currency` from `LINK.GROUPS.gear` (coins are personal; inventory/bag/weapons still sync). (2) **Condition pills unreadable** — `.cond button` (24×24 circle) out-specified `.cond-name`, crushing the name button into a circle over the −/✕; scoped to `.cond button:not(.cond-name)`. (3) Removed the redundant **"paste" button** from the Spellbook header (paste still reachable via "+ hand-add" and via tapping a greyed spell).
- **v58:** tidied the SW registration — was `sw.js?v=47` (stale literal); now plain `sw.js` (browser re-checks it every load; the SW's `CACHE` const drives updates). Nothing to keep in sync.
- **v59 — Summons picker overhaul + boost details:** Add-summon picker now has tabs (All/Known/Favorites), a type filter (beast/fey/…), CR sort (↑/↓/A–Z via `crToNum`), and ★/✓ marks per row (`ch.summonFav`/`ch.summonKnown`, synced in mental group). Tapping a creature opens its full stat-block **preview card** (`summonPreview` → `statBlockBody`) with an Add button — see before you summon. "Known" auto-fills on add (`markKnown`). Removed dead `summonStub` (its link used `wikidotSpellUrl` on a creature — wrong; **wikidot has NO monster pages**). Summon cards now show **Mighty Summoner / Undead Thralls boost explanations** at the bottom when toggled (`.sm-boost`). `SUMMON_TYPES` (note: `CREATURE_TYPES` already exists in views.js for icons).
- **Wikidot scope note (re Mathias's "link+paste for races/items/weapons/classes"):** dnd5e.wikidot.com has pages for **spells** (`/spell:x`, done), **classes** (`/druid`), **subclasses** (`/druid:circle-of-the-moon`), **races** (likely `/lineage:` or top-level), **feats**, **magic items** (`/wondrous-items:...`) — but **NOT monsters**. So summons can't link there. The others are feasible but each needs its own URL scheme + paste parser + a home in the app (weapons/items exist; races/classes don't have a structured entity yet). NOT yet built — needs scoping with Mathias.
- **v60 — reload/nav fixes:** (1) **Remembers last screen + tab** — `render()` persists `{screen,tab}` to `localStorage["grimoire.nav.v1"]`; `boot()` restores it (sheet/summons/session + tab) instead of resetting to Stats (fixes "iOS killed the PWA → I start over"). (2) **Reload button that did nothing** — the update toast used a captured `_swReg.waiting` that could be stale, falling through to a plain reload of the SAME cached version; `doUpdate` now fetches the registration fresh, messages waiting OR installing, and has a 1.5s fallback reload. (3) Summons screen gained a **humanoid icon back-button** (same 64px tile as the Combat→Summons button) → returns to the sheet.
- **v61:** modal scroll-lock — opening a prompt (e.g. Take damage) auto-focuses an input; the keyboard popping scrolled the sheet behind and left it scrolled. `modal()`/`closeModal()` now freeze the body (`position:fixed` at offset) while open and restore scroll on close (`lockScroll`/`unlockScroll` in util.js).
- **v62:** char ⋯ menu regrouped into labelled sections (Build / Look / At the table / Backup) instead of a flat 9-item list; added a **Kill count: shown/hidden toggle** (global pref `localStorage["grimoire.killcount"]`, default shown; `killsShown()` gates the Notes-tab kill-count section).
- **v63 — Feats & magic items: Wikidot look-up + paste (DONE).** Feature form (→ ch.features) and inventory item form (→ ch.inventory) each got a "Look up '<name>' on the Wikidot ↗" link (`wikidotFeatUrl` = `feat:<slug>`, `wikidotItemUrl` = `wondrous-items:<slug>` — uniform for all magic items incl. rings/weapons) + a "Paste … text ↧" box. Shared `parseEntryText` (first non-empty line = name, rest = body, drops "Source:") + `openPasteEntry`/`pasteEntryDo`; `pasteFeat`/`pasteItem` refill the open form preserving its bonus/adv rows. **Items gained a Notes/description field** (`it.notes`, surfaced in `renderItemForm`/`itemCapture`/`itemSave`, shown via `.it-notes` on the gear list). Numeric bonuses stay manual. `wikidotSlug` helper shared by spell/feat/item URLs.
- **IDEA (Mathias, "for later" — brainstorm before building): a DM feature in the app.** Not scoped yet; Mathias wants to brainstorm the feature set together in a future session. Possible directions to explore then: encounter/initiative tracker, monster stat-block library (the bundled summons.json already has 334 SRD creatures + the new picker), party overview / shared NPC notes, quick monster HP trackers, session prep notes. Decide scope WITH him first.
- **v64 — spell fixes:** (1) **Custom spells are now editable** — spell detail shows "Edit / re-paste" + "Delete" for `s.custom`; `customForm`/`csRender`/`csCapture` (add form doubles as edit), `customSave` updates in place (same id, keeps known/prepared). **"Paste new text to fill these in"** (`pasteSpellFill`→`openSpellPaste`→`spellPasteDo`) re-parses pasted text into the fields so a botched paste is fixable, not just deletable; "At higher levels" folds into desc. `customDelete` clears it from known/prepared/favorites. (2) **Search-clear (✕)** fired a `search` event nothing listened for → added a `search` listener mirroring `input`. (3) Level dropdown verified working — the report was a stale cached build (v60 reload fix delivers updates).
- **DESIGN QUEUED — Druid Wild Shape (Mathias asked "how best to implement?"):** wants it like Summons — transform into a beast, track its HP, see stats/details, with Favorites/Known/all-eligible forms + an "allow beyond limit" escape hatch, while keeping access to the normal combat tab. KEY: reuse the summons infra (beast library `summons.json`, the v59 picker w/ tabs+CR-sort+type-filter+stat preview, `statBlockBody`, per-instance HP). CR limit from level+subclass: non-Moon L2≤¼/L4≤½/L8≤1 (+move restrictions); Circle of the Moon = max(1, floor(level/3)), bonus-action shift, elementals at L10. Store e.g. `ch.wildShape={active, known[], fav[]}`; entry from Resources/Combat; while shaped show a beast card + a "revert" banner, normal tabs still reachable; beast HP pool reverts at 0 with carry-over. Proposed to Mathias — awaiting go-ahead.
- **v65 — Transformation engine (Wild Shape + Polymorph) + auto-update:** `ch.shape = {active, fav, known}`. Shared engine (reuses summons: `makeSummon`, picker, `statBlockBody`, per-instance HP). `SHAPE_SOURCES` config — only the eligible-filter + cost differ. **Wild Shape:** Combat banner/button for Druid L2+; auto-adds a "Wild Shape" uses tracker to Resources (`ensureWildShape`, flag `wildshape`, 2/short); CR by level/subclass (`wildShapeMaxCR`; non-Moon L2≤¼/L4≤½/L8≤1, Moon=max(1,⌊L/3⌋), elementals L10, Moon `shapeHeal`). **Polymorph:** button on Polymorph/True Polymorph spell detail (`polymorphStart`); any beast CR≤totalLevel, concentration. Dedicated `viewShape` screen + `shapeCard` + humanoid back-button; `shapeBanner` on Combat. Damage-to-0 → `revertShape` carries overflow to real HP. Picker = `shapePicker`/`shapePickRows` (Eligible/Known/Favorites/Allow-any). **Auto-update:** removed the "Reload" prompt — new SW now activates + reloads automatically (`autoUpdate`; safe because nav state persists from v60); 30-min checks.
- **v66 — Arrange drag fix:** drag-to-reorder felt "one step at a time" (row never left flow, never followed finger). Rewrote `dragStart/Move/End`: the row lifts out (`position:fixed`, follows finger) with a dashed `.drag-placeholder` showing the drop spot; lands exactly where released, any distance in one drag.
- **Future transform reuse (noted):** True Polymorph (wider creature pool — same engine), Shapechange (keeps your own HP — variant). Elemental forms already wired for Moon L10.
- **v67:** Proficiencies "Done" now saves the typed-but-not-Added entry (`profDone`) instead of dropping it.
- **v68 — DM MODE v1 (research-backed):** ran a deep-research workflow (101 agents, 19 sources, adversarially verified) on what DMs need → built a "DM mode" (home button, any copy; `DM = {encounters, active, settings}` in `localStorage["grimoire.dm.v1"]`). **Encounters:** build/save named encounters (monsters+counts), launch; "Quick fight"; "Resume combat" persists across closes. **Combatants:** monsters from the library (added as ONE group row with per-instance HP — fast-combat trick), saved characters (`dmCharCombatant` pulls AC/HP), or custom (non-app players). **Tracker (`viewDMRun`/`dmCombatantRow`):** auto-sort by initiative (`dmSorted`), round counter, current-turn highlight, Next/Start (`dmNext`, ticks until-your-next-turn condition timers), tap-to-damage/heal (temp-HP aware), conditions with optional round timers, **concentration flag + reminder on damage** (dice-free), **bloodied** (≤½), **undo-last** HP change, tap-monster→stat block. **DM features menu (`dmSettings`/`DM_FEATURES`/`dmOn`):** every feature toggleable (avoids feature-bloat anti-pattern). All offline. Screens route via `ui.screen` dm/dmEdit/dmRun (no scene). **NEXT: live link-pull of party HP/AC** — the research's #1 differentiator (D&D Beyond's most-cited gripe is HP not auto-carrying); the linking infra exists. Non-app players already covered by custom combatants.
- **v69 — DM mode live link-pull (the research's #1 differentiator):** "+ PC → Add by link code (live)" pulls a player's shared snapshot via `LINK.fetchSnapshot(code)` and builds a `kind:"link"` combatant with their real HP/temp/conditions; AC computed from the shared abilities+combat+gear through `Calc.armorClass` (`dmComputeAC`). Per-combatant ↻ refresh (`dmRefreshLink`) + a "↻ Sync PCs" run-bar button (`dmSyncAll`) re-pull live; green "live" badge. **Read-only — never pushes back** to the player's sheet (so it can't clobber their data). Non-app players still use Add custom. Verified end-to-end against the live relay (push linked Fighter → pull AC 18 / 31·44+5 / Frightened → player edits + re-push → refresh shows 12/44 + Prone/Poisoned + auto-bloodied).
- **v70 — DM gifts (give items/pictures/drawings to linked players):** Worker gained a `/gift/CODE` queue keyed by the recipient's **link code** (send/pull, capped 40, 90-day TTL) — **deployed**. DM side: a linked combatant's ⋯ menu → Give item (form) / Give picture (file, downscaled 900px) / Give drawing (`openDrawPad`), sent via `GIFT.send(linkCode, …)`. Player side: `GIFT` in link.js polls own link code on boot/visibility/30s; items → inventory (toast), images → IndexedDB (`Media`) + `ch.dmGifts` shown in a **"From your DM" gallery on the Notes tab** (`dmGiftView`/`dmGiftDelete`; sheet now hydrates `img[data-mid]`). Read-only channel; reuses the party-relay pattern. Verified end-to-end vs the live relay.
- **Still open:** no light-mode scene variant (light = plain by design).
- **NEXT (Mathias, for later) — 2014 vs 2024 edition must NOT mix:** picking an edition should fully scope rules + abilities + spells + everything to that edition; today only the spell *lists* differ by edition (`data/spells-2014.json` vs `-2024.json`), but class rules/slots/DC/feature logic are edition-agnostic (`calc.js`, `rules.js`), and subclass/feat/species handling doesn't branch on edition. Audit everywhere `ch.edition` should gate behaviour (rules, class features, ASIs/feats, weapon mastery, species traits, prepared-vs-known changes in 2024, etc.) and make the two editions self-consistent.
