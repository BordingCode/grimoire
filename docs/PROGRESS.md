...
- **QUEUED (Mathias said "start there"): wikidot look-up + paste for MAGIC ITEMS and FEATS.** Research done — both have uniform wikidot schemes: feats `https://dnd5e.wikidot.com/feat:<slug>`; magic items (incl. rings/weapons) `https://dnd5e.wikidot.com/wondrous-items:<slug>`. Feat paste = name + "Source:" + description (bullets); item paste = name + "Source:" + "Wondrous item, rarity (attunement)" line + description. Plan: `wikidotFeatUrl`/`wikidotItemUrl` helpers + a look-up link & paste box in the feature form (→ ch.features) and the inventory item form (→ ch.inventory); numeric bonuses stay manual (too varied to auto-parse). NOT yet built — interrupted by newer requests.
